...
export interface RunFileMessageEncoderDeps {
  ...
  }) => Promise<{ text: string | undefined; wasTruncated: boolean }>;
}
...
export function createRunFileMessageEncoder(
  ...
  async function encode(files: TFile[], agentId: string): Promise<BaseMessage[]> {
    ...
    const [imageResult, documentResult, audioResult, videoResult, extractResult] =
      await Promise.all([
        ...
        textFiles.length > 0
          ...
          : Promise.resolve({ text: undefined, wasTruncated: false }),
      ]);
