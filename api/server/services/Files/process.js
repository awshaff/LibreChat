...
const processDeleteRequest = async ({ req, files }) => {
  ...
  if (deletedFileIds.length > 0) {
    ...
    if (metadataDeletedFileIds.length > 0) {
      ...
      try {
        await db.removeProjectResourceFilesFromAllProjects(metadataDeletedFileIds);
      } catch (error) {
        logger.error('Error cleaning up orphaned project file references', error);
      }
    }
  }
 ...
 * @param {ServerRequest} params.req - The Express request object.
 * @param {Express.Response} params.res - The Express response object.
 * @param {FileMetadata & { project_id?: string }} params.metadata - Additional metadata for the file.
 * @param {import('@librechat/api').UploadSseStream | null} [params.sseStream] - Active upload SSE stream, if enabled.
 * @returns {Promise<void>}
 */
const processProjectFileUpload = async ({ req, res, metadata, sseStream }) => {
  const { file } = req;
  const appConfig = req.config;
  const { project_id, tool_resource, file_id, temp_file_id = null } = metadata;
  ...
  if (!project) {
    throw new Error('Project not found for project file upload');
  }
